...
public class NativeSection extends JPanel {
	...
	final TitledBorder titledBorder = new TitledBorder(etchedBorder, "") {
	    ...
	    boolean _computeIntersection(Rectangle dest,
		...
		switch (titlePos) {
			...
			grooveRect.y += diff;
			grooveRect.height -= diff;
			_textLoc.y = (grooveRect.y - descent)
			...
			break;
		    case ABOVE_BOTTOM:
			_textLoc.y = (grooveRect.y + grooveRect.height)
				     - (insets.bottom + descent + TEXT_SPACING);
			break;
		    case BOTTOM:
			grooveRect.height -= fontHeight / 2;
			_textLoc.y = ((grooveRect.y + grooveRect.height) - descent)
				     + ((ascent + descent) - insets.bottom) / 2;
			break;
		    case BELOW_BOTTOM:
			grooveRect.height -= fontHeight;
			_textLoc.y = grooveRect.y + grooveRect.height + ascent
				     + TEXT_SPACING;
			break;
		}
		...
		switch (justification) {
			...
			break;
		    case RIGHT:
			_textLoc.x = (grooveRect.x + grooveRect.width)
				     - (stringWidth + TEXT_INSET_H + insets.right);
			break;
		    case CENTER:
			_textLoc.x = grooveRect.x
		...
		if (border != null) {
		    if (((titlePos == TOP || titlePos == DEFAULT_POSITION)
			 && (grooveRect.y > _textLoc.y - ascent))
			|| (titlePos == BOTTOM
			    && (grooveRect.y + grooveRect.height < _textLoc.y + descent))) {
			...
			clipRect.setBounds(saveClip);
			if (_computeIntersection(clipRect, _textLoc.x + stringWidth + 1, y,
				x + width - (_textLoc.x + stringWidth + 1), height)) {
			    g.setClip(clipRect);
			    border.paintBorder(c, g, grooveRect.x, grooveRect.y,
				    grooveRect.width, grooveRect.height);
			}
			...
			if (titlePos == TOP || titlePos == DEFAULT_POSITION) {
			    ...
			    clipRect.setBounds(saveClip);
			    if (_computeIntersection(clipRect, _textLoc.x - 1, _textLoc.y + descent,
				    stringWidth + 2, y + height - _textLoc.y - descent)) {
				g.setClip(clipRect);
				border.paintBorder(c, g, grooveRect.x, grooveRect.y,
					grooveRect.width, grooveRect.height);
			    }
			    ...
			    clipRect.setBounds(saveClip);
			    if (_computeIntersection(clipRect, _textLoc.x - 1, y,
				    stringWidth + 2, _textLoc.y - ascent - y)) {
				g.setClip(clipRect);
				border.paintBorder(c, g, grooveRect.x, grooveRect.y,
					grooveRect.width, grooveRect.height);
			    }
		...
		g.drawString(getTitle(), _textLoc.x, _textLoc.y);
