...
public class NativeSuiteHorizontal extends JPanel {

    private Vector<Widget> items = new Vector<Widget>();
    private Vector<JamLeft> containers = new Vector<JamLeft>();
    private JamLeft root = new JamLeft();
    private JamLeft last = root;

    public NativeSuiteHorizontal(SuiteHorizontal unit, Theme ntheme) {
        final NativeSuiteHorizontal me = this;
        new BiSet<Widget>(unit.suite()) {

            public void onAdd(Widget item) {
                JamLeft newContainer = new JamLeft();
                newContainer.left().set(item);
            ...
            public void onDrop(Widget item) {
                for (int i = 0; i < items.size(); i++) {
                    if (items.get(i) == item) {
                        ...
                        containers.get(i).left().set(null);
                        if (i < items.size() - 1) {
